...
- IOS: open ```.xcworkspace``` with ``` Xcode ```. Right click ```Fonts``` folder. Add file to learn_fc. Select all fonts from ```src/app/assets/fonts```. DON'T TICK ```copy items if needed```. then save.
